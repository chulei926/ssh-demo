...
APP_NAME=com.example.ssh.server.demo.SshDemoServerApplication
...
help() {
  echo "Usage: app.sh [start|stop|restart|status|dump]"
  exit 1
}
...
start(){
  ...
  if [ $? -eq "1" ]; then
    ...
    if [ "$IS_DOCKER_ENV" = "true" ]; then
      $JAVA_CMD $CONFIG_FILES $JAVA_MEM_OPTS -Duser.timezone=GMT+8 -Duser.language=zh -cp $CLASSPATH $APP_NAME > start.log 2>&1
    else
      nohup $JAVA_CMD $CONFIG_FILES $JAVA_MEM_OPTS -Duser.timezone=GMT+8 -Duser.language=zh -cp $CLASSPATH $APP_NAME  > start.log 2>&1 &
      check
    fi
  fi
}
